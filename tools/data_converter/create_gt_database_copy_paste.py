...
def crop_image_patch_based_on_bbox(pos_proposals, org_img):
    num_pos = pos_proposals.shape[0]
    img_patches = []
    for i in range(num_pos):
        bbox = pos_proposals[i, :].astype(np.int32)
...
def create_groundtruth_database(dataset_class_name,
    ...
    elif dataset_class_name == 'NuScenesMultiView_Map_MultiModalDataset':
        dataset_cfg.update(
            with_box2d=True,
            modality=dict(
                use_lidar=True,
    ...
    if with_mask:
        ...
        for i in imgIds:
            ...
            file2id.update({osp.basename(info['file_name']): i})
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        if with_mask:
            # prepare masks
            gt_boxes = annos['bboxes']

            #print(gt_boxes)

            for iter, img_info in enumerate(example['img_info']):

                current_2d_bboxes = gt_boxes[iter] #only use the 2d bboxes on the current image
                num_obj2D += len(current_2d_bboxes)
                
                img_path = osp.basename(img_info['filename'])

                if img_path not in file2id:
                    print(f'Skipping image {img_path} for empty mask: not found in file2id')
                    continue
                img_id = file2id[img_path]

                # Get annotation IDs for the specified image ID
                image_id_int = coco.getImgIds(imgIds=[img_id])
                kins_annIds = coco.getAnnIds(imgIds=image_id_int)

                
                kins_raw_info = coco.loadAnns(kins_annIds)
                kins_ann_info = _parse_coco_ann_info(kins_raw_info)

                bbox_iou = bbox_overlaps(kins_ann_info['bboxes'], current_2d_bboxes)
                
                valid_inds = (bbox_iou.max(axis=0) > 0.5)

                image = mmcv.imread(img_info['filename'])
                object_img_patches = crop_image_patch_based_on_bbox(current_2d_bboxes, image)


                all_object_img_patches.append(object_img_patches)
                all_valid_inds.append(valid_inds)

            # Method 1: Using list comprehension
            all_objects_flattened = [image for sublist in all_object_img_patches for image in sublist]

            
        
        print(f"num_obj: {num_obj}")
        print(f"num_obj2D:{num_obj2D}")
        print(f"all_object_img_patches: {len(all_objects_flattened)}")
        # if len(all_objects_flattened) > 0:
        #     for patch in all_objects_flattened:
        ...
        for i in range(num_obj):
            ...
            if with_mask and all_object_img_patches.shape[0] > 0:
                if not all_valid_inds[i]:
                    # Skip object for empty or invalid mask
                    print("Skip object for empty or invalid mask")
                    continue
                img_patch_path = abs_filepath + '.png'
                mmcv.imwrite(all_object_img_patches[i], img_patch_path)
            ...
            if (used_classes is None) or names[i] in used_classes:
                ...
                # if with_mask:
                #     db_info.update({'box2d_camera': flattened_gt_boxes[i]})
    ...
    #         gt_boxes = annos['bboxes']
    #         print(f"example['img_info']: {example['img_info']}")
    #         #print(f"example['img_info']['filename']: {example['img_info']['filename']}")
