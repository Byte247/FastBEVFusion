...
class FastBEVFusionTransfusionheadPillar(BaseDetector):
    ...
    def extract_feat(self, img, img_metas, mode):
        ...
        #image_network_time = start_event.elapsed_time(end_event)
        #print(f'Time for image_network: {image_network_time:.3f} ms')
        ...
        #torch.cuda.synchronize()
        #view_transform_time = start_event.elapsed_time(end_event)
        #print(f'Time for view_transform: {view_transform_time:.3f} ms')
    ...
    def extract_pts_feat(self, pts):
        ...
        #torch.cuda.synchronize()
        #voxelize_time = start_event.elapsed_time(end_event)
        #print(f'Time for voxelize: {voxelize_time:.3f} ms')
        ...
        #torch.cuda.synchronize()
        #lidar_network_time = start_event.elapsed_time(end_event)
        #print(f'Time for lidar_network: {lidar_network_time:.3f} ms')
    ...
    def forward_train(
        ...
        feature_bev = self.fusion_module(lidar_features, feature_bev) # this framework requires features inside lists for some reason.
